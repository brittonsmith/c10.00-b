...
On most systems \cdFilename{stderr} can be directed to a file by redirection of 2 as in the following example
\begin{verbatim}
...
\item[set atomic data ion sulphur DR -] \cdTerm{sulphur dielectronic
recombination rates}.
...
The default, which can also be set with the keyword \cdCommand{mix}, is to use the
larger of the \citet{Badnell1991} or guessed rates.  The keyword \cdCommand{pure} says to
use only the Badnell rates.  The keyword \cdCommand{scale} says to use scaled oxygen
...
The \cdCommand{set Chianti} command by itself will turn them on,
using the species contained in the
\cdFilename{CloudyChianti.ini} file in the 
...
This makes it possible to create \cdFilename{ini} files that select portions
of Chianti.
The \cdCommand{no hybrid} option disables the Opacity Project data and only uses Chianti.
The \cdCommand{print} option will print which Chianti species are being used and the number of levels.  

The \cdCommand{set lamda} command, described on \pageref{sec:SetLamda},
...
This sets the secondary ionization rate due to supra-thermal electrons
...
These commands affect the electron density or how it is determined.
\begin{description}
...
Note:  The \cdCommand{eden} command sets an extra source
of electrons.
...
This changes the treatment of hydrogen-oxygen charge transfer.
Charge
...
\cdCommand{ionization} option to function properly.
...
The \cdCommand{set Chianti} command, described on page \pageref{sec:SetChianti},
...
One of the keywords \cdCommand{dielectronic} or \cdCommand{radiative}
must appear.
This changes
...
The \cdCommand{kludge} option modifies the treatment
...
Fits to Davidson's results are used if \cdCommand{Burgess on} appears.
The process is assumed to be 100\% efficient if \cdCommand{Burgess off} appears.
The
...
Fits to Davidson's results are used if \cdCommand{Nussbaumer on} appears.
The
process is assumed to be 100\% efficient if \cdCommand{Nussbaumer off} appears.
The
...
The old
approximations will be used if the \cdCommand{Badnell off} option appears.

The \cdCommand{print} option will output both RR and DR rates
...
The \cdCommand{Badnell on} option
is the default and tells the code to use the rate coefficients
calculated by Badnell et al. in a series of papers that
appeared in A\&A.
The old
approximations will be used if the \cdCommand{Badnell off}
option appears.

The \cdCommand{print} option will output both RR and DR rates for
...
This modifies the behavior of the \cdCommand{save spectrum} command.
A series of \cdCommand{set spectrum} commands can occur
and these modify any \cdCommand{save spectrum} commands
that follow them.
The following shows an example of
changing the resolution and then producing
a \cdCommand{contin.dat} file.
\begin{verbatim}
...
\cdCommand{set spectrum} command.
\begin{description}
...
The \cdCommand{no UTA} command turns off UTA ionization.
...
\cdTerm{N.B.!!}  The fidelity of the simulation is compromised!
...
The \cdCommand{element limit} command provides a way
...
\subsection{Turn off grain physics}
...
\cdTerm{Turning off all grain physics}.  All grain-gas interactions,
except for their opacity, are turned off with the
\cdCommand{no grain physics} command.
Diffuse emission of the grains is no longer included so energy
is not conserved.
The dynamical and opacity effects of the grains will
still be included.

\cdTerm{Grains no reevaluate}.  The \cdCommand{no reevaluate}
option on the \cdCommand{grains} command
...
The \cdCommand{no line transfer} command turns off line transfer
...
The \cdCommand{no ionization reevaluate} command tells
...
The \cdCommand{no opacity reevaluation} command tells the
...
\cdCommand{no fine opacities} command.
...
This is not included in the \cdFilename{fast.ini} file since it
...
(with the \cdCommand{compile grains} command) to use grains.
