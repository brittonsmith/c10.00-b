...
\noindent This gives the continuum summarized by
\citet{Davidson1985}.
This is the net observed continuum, originating in both the pulsar and
nebula, and not the pulsar continuum alone.
The continuum is given in Table \ref{tab:CrabContinuum},
which gives the observed flux at Earth
(erg cm$^{-2}$ s$^{-1}$ Hz$^{-1}$).

\begin{table}
\centering
\caption{Crab Continuum}
\label{tab:CrabContinuum}
\begin{tabular}{ll}\hline
$\nu$(Ryd)& $f_\nu$\\
\hline
1.0($-$5)& 3.77E-21\\
...
According to Davidson and Fesen, the total luminosity of the Crab is
$L_{tot} = 10^{38.14}$ erg s$^{-1}$.
The Crab continuum could be generated by combining
the commands
\begin{verbatim}
...
The heavy line is the \cdCommand{table Draine} continuum. }
\end{figure}
...
``log g'', as a second parameter.  The ``1og'' in ``log g'' would cause the
temperature to be interpreted as the log of the temperature.  Either
do not enter the string ``log g'' or write it as ``logg''.
...
(\citealp{Leitherer1999}).
The \cdCommand{table starburst} command, which existed for this purpose in versions through C07, has been
replaced.
The Starburst99 SEDs have been unified with the stellar
atmosphere treatment.
The procedure is to store the Starburst99 output in
a file with a ``\cdFilename{.stb99}''  or ``\cdFilename{.stb}''
extension and compile it as in the following example
\begin{verbatim}
compile star "some_file.stb99"
\end{verbatim}
This will produce files \cdFilename{some\_file.ascii} and \cdFilename{some\_file.mod}, which can then
be used in the \cdCommand{table stars} command:
\begin{verbatim}
table star "some_file.mod" age=6.7
\end{verbatim}
Multiple Starburst99 SEDs can be combined.
The \cdCommand{table stars} command is described next.
...
file should be formatted. The resulting file should then be compiled using
\begin{verbatim}
