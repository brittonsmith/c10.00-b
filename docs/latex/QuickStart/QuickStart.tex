...
\Huge
\emph{Quick Start Guide to \Cloudy\ C10}\\
\begin{figure}
\begin{center}
\includegraphics[clip=on,width=\columnwidth,height=0.8\textheight,keepaspectratio]{helvan63.jpg}
\end{center}
\label{fig:header}
\end{figure}
...
\noindent
{\small
{\em Cover image:} \copyright\ 2009 -- Steven de Jong. Reproduced with permission.\\
\vspace{1em}
\noindent
This image depicts a skater participating in the {\it elfstedentocht} (eleven
cities tour), a famous 200~km skating tour held on natural ice in the province
of Friesland in the Netherlands in years where the winter is cold enough. The edition of 1963
is infamous for its bad weather conditions: temperatures below $-10\
^\circ$C at the start, strong winds, up to 40~cm of snow, and bad ice. Conditions were so bad that from the 10096
people that started, only 129 managed to finish the tour within the set
time limit (56 of them competition skaters). This story was recently turned into
a movie called {\it De Hel van '63} (The Hell of '63) by Steven de Jong. The cover
image was taken from this movie.}
\clearpage
...
described by \citet{FerlandEtAl98}.''
...
\end{verbatim}
\normalsize
where the file \cdFilename{sim.in} contains the
input commands and the main output will
go to the file \cdFilename{sim.out}.  This method
of running the code is described in
the chapter \cdTerm{Running a Single Model} in Part~2 of \Hazy.

I also keep a \cdFilename{run} script on my path.
In Unix the contents of the \cdFilename{run} file are the following
\small
\begin{verbatim}
/u/home1/gary/cloudy/trunk/cloudy.exe < $1.in > $1.out
\end{verbatim}
\normalsize
and in Windows the following is in \cdFilename{run.bat}:
\small
\begin{verbatim}
c:\projects\cloudy\trunk\debug\cloudy.exe < %1.in > %1.out
\end{verbatim}
\normalsize
In both cases the code could be run with
the command \cdCommand{run sim}.  The code
would read commands from the file \cdFilename{sim.in}
and write its output to the file \cdFilename{sim.out}.
...
Generally this will be near the \hplus -- \h0 ionization front.
...
of $10^{22}\ \pscm$.
This is large enough for an \hplus -- \h0\
...
by photoexcitation of \h0\ by the continuum in the Lyman lines, followed by
decay into the metastable $2s$ \h0\ level, which is then photoionized by the Balmer
continuum.
...
it exceeds the final value.  The grid is equally spaced in log space since
...
The \h0\ region has a mean temperature a bit under $10^3$~K,
...
to $\sim 500 \K$ in the small \h0\ region,
