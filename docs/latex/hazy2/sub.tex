...
\cdTerm{cdMPI}  When the code is executed using MPI (Message Passing Interface,
used on parallel machines)  the code must call a specific exit handler,
MPI\_finalize, upon exit.
Routine \cdRoutine{cdMPI} should be called after
\cdRoutine{cdInit} but
...
When routine \cdRoutine{cdGetLineList} is called it uses these
...
If the last parameter is \cdVariable{true} then emergent
