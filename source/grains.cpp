...
void GrainsInit(void)
{
	...
	for( size_t nd=0; nd < gv.bin.size(); nd++ )
	{
		...
		for( ns=0; ns < gv.bin[nd]->sd.size(); ns++ )
		{
			...
			sptr->ipLo = hunt_bisect( rfield.anu, rfield.nupper, (realnum)Ethres ) + 1;
...
STATIC double GrnStdDpth(long int nd)
{
	...
	if( gv.bin[nd]->nDustFunc == DF_STANDARD )
	{
		if( gv.bin[nd]->matType == MAT_PAH || gv.bin[nd]->matType == MAT_PAH2 )
		{
			...
			{
				/* the scale factor is the total of the hydrogen atomic and molecular fractions, small when gas is ionized
				 * and unity when atomic or molecular. This function is observed for PAHs
				 * across the Orion Bar, the PAHs are strong near the ionization front and
				 * weak in the ionized and molecular gas */
				/* >>chng 04 sep 28, propto atomic fraction */
	...
	GrnStdDpth_v = min(max(1.e-10,GrnStdDpth_v),1.);
...
STATIC void GrainChargeTemp(void)
{
	    ...
	    fabs(gv.TotalEden-oldTotalEden) > CHRG_TOLER*dense.eden )
	{
		...
		else if( fabs(gv.GasHeatNet-GasHeatNet) > HEAT_TOLER*thermal.ctot )
		{
			...
			conv.BadConvIoniz[0] = gv.GasHeatNet;
			conv.BadConvIoniz[1] = GasHeatNet;
		}
		       ...
		       long ipStart)
{
	...
	if( ipStart == 0 )
	{
		...
		gv.bin[nd]->chrg[nz]->ipThresInfVal =
			hunt_bisect( rfield.anu, rfield.nupper, (realnum)gv.bin[nd]->chrg[nz]->ThresInfVal ) + 1;
	}
	...
	if( ipStart == 0 )
	{
		...
		gv.bin[nd]->chrg[nz]->ipThresInf =
			hunt_bisect( rfield.anu, rfield.nupper, (realnum)gv.bin[nd]->chrg[nz]->ThresInf ) + 1;
	}
			...
			for( i=0; i < rfield.nflux; i++ )
			{
				...
				for( nz=0; nz < gv.bin[nd]->nChrg; nz++ )
				{
					...
					if( gptr->DustZ <= -1 && i >= ipLo )
						gv.dstab[i] +=
							gptr->FracPop*gptr->cs_pdt[i]*gv.bin[nd]->dstAbund;
				}
			}
		}
		for( i=0; i < rfield.nflux; i++ )
			     ...
			     /*@out@*/ double *hla)
{
	...
	*hla = 0.;
	...
	ipLya = Transitions[ipH_LIKE][ipHYDROGEN][ipH2p][ipH1s].ipCont - 1;
			     ...
			     /*@out@*/ realnum *dccool)
{
	...
	Accommodation = 2.*gv.bin[nd]->atomWeight*WeightMol/POW2(gv.bin[nd]->atomWeight+WeightMol);
#ifndef IGNORE_GRAIN_ION_COLLISIONS
	CollisionRateMol += Accommodation*findspecies("CO")->hevmol*
		sqrt(8.*BOLTZMANN/PI/ATOMIC_MASS_UNIT/WeightMol*phycon.te);
#else
	CollisionRateMol = 0.;
#endif
	...
	size_t nd)
{
	...
	/* This routine must return a scale factor > 0 and <= 1 for the grain abundance at this position.
	...
	return min(max(1.e-10,GrnVryDpth_v),1.);
}
