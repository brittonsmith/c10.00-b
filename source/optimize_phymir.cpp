		     ...
		     realnum toler)
{
	...
	phymir_mode mode = optimize.lgParallel ? ( cpu.lgMPI() ? PHYMIR_MPI : PHYMIR_FORK ) : PHYMIR_SEQ;
	long nCPU = optimize.lgParallel ? ( cpu.lgMPI() ? cpu.nCPU() : optimize.useCPU ) : 1;
