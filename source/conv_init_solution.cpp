		...
		double *dCoolNetDT )
{
	...
	while( loop < LOOP_MAX && !lgConvergeCoolHeat && !lgAbort )
	{
		...
		lgConvergeCoolHeat = lgCoolHeatCheckConverge( CoolNet );
...
STATIC void ChemImportance( void )
{
	...
	{
		/* is element included in the chemistry, and is it a molecule? */
	...
	for(i=0;i<mole.num_comole_calc;++i)
	{
		OxyInGrains += (1 - COmole[i]->lgGas_Phase)*COmole[i]->hevmol*COmole[i]->nElem[ipOXYGEN]; 
	}
...
int ConvInitSolution(void)
{
	...
	else
	{
		...
		* temperature calculation if temperature falls below floor */
		...
		for( ionlup=0; ionlup<2; ++ionlup )
		{
			...
			dCoolNetDTOld = 0.;
			lgConvergeCoolHeat = lgCoolNetConverge( &CoolNet , &dCoolNetDT );
			if( lgAbort )
				break;
		...
		while( !lgConvergedLoop && LoopThermal < LIMIT_THERMAL_LOOP )
		{
			...
			if( fabs(CoolNet)< SMALLDOUBLE )
				...
				lgConvergedLoop = true;
			else if( fabs(CoolNet)/MAX2( thermal.ctot , thermal.htot ) <conv.HeatCoolRelErrorAllowed*10. )
				lgConvergedLoop = true;
	...
	if( (strcmp(dense.chDenseLaw,"WIND") == 0) && !( wind.lgBallistic() || wind.lgStatic()) )
	{
