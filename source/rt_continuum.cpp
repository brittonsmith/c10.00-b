...
#	endif
	...
	if( rfield.lgOpacityFine )
	{
		...
		for( long i=0; i<rfield.nfine; ++i )
		{
			rfield.fine_opt_depth[i] += 
				rfield.fine_opac_zone[i]*(realnum)radius.drad_x_fillfac;
		}

		// evaluate fine opacity transmission coefficient for transmission
		// by scattering opacity
		if( opac.lgScatON )
		{
			/* sum over coarse continuum */
			for( long i=0; i < rfield.nflux-1; i++ )
			{
				...
				if( rfield.ipnt_coarse_2_fine[i] && rfield.ipnt_coarse_2_fine[i+1] )
				{
					...
					for( long j=rfield.ipnt_coarse_2_fine[i]; j<rfield.ipnt_coarse_2_fine[i+1]; ++j )
						rfield.trans_coef_zone[i] += 1.f / 
						(1.f + rfield.fine_opac_zone[j]*(realnum)radius.drad_x_fillfac );
					...
					else
					{
						...
						// just use first cell
						rfield.trans_coef_zone[i] = 1.f / 
							(1.f + rfield.fine_opac_zone[rfield.ipnt_coarse_2_fine[i]]*
							(realnum)radius.drad_x_fillfac );
					}
				}
				else
				{
					// this branch, not within find continuum energy range 
					// transmission unity 
					rfield.trans_coef_zone[i] = 1.f;
				}

				// the total is just the current total times the transmission 
				// coefficient in this zone
	...
	if( rfield.flux[0][iso.ipIsoLevNIonCon[ipH_LIKE][ipHYDROGEN][0]-1]>SMALLFLOAT &&
		(rfield.flux[0][iso.ipIsoLevNIonCon[ipH_LIKE][ipHYDROGEN][0]-1]/
		SDIV(rfield.flux_total_incident[0][iso.ipIsoLevNIonCon[ipH_LIKE][ipHYDROGEN][0]-1]) ) > SMALLFLOAT && 
		!opac.lgScatON &&
		radius.depth/radius.Radius < 1e-4 )
	{
		...
		* error found by Peter van Hoof */
		double tau_effec = -log((double)rfield.flux[0][iso.ipIsoLevNIonCon[ipH_LIKE][ipHYDROGEN][0]-1]/
			(double)opac.tmn[iso.ipIsoLevNIonCon[ipH_LIKE][ipHYDROGEN][0]-1]/
			(double)rfield.flux_total_incident[0][iso.ipIsoLevNIonCon[ipH_LIKE][ipHYDROGEN][0]-1]);
