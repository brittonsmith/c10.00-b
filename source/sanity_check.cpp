...
STATIC void SanityCheckBegin(void )
{
	...
	for( nelem=0; nelem<LIMELM; ++nelem )
	{
		...
		{ 
			...
			if( fabs(Aul - Transitions[ipH_LIKE][nelem][ipH2p][ipH1s].Emis->Aul ) /Aul > 0.01 )
			{
	...
	for( nelem=1; nelem<LIMELM; ++nelem )
	{
		...
		if( iso.numLevels_max[ipHE_LIKE][nelem] > 8 && dense.lgElmtOn[nelem])
		{	
			/* following used to print current values of As */
			/*fprintf(ioQQQ,"%.2e\n", Transitions[ipHE_LIKE][nelem][9][1].Emis->Aul );*/
			if( fabs( as[nelem] - Transitions[ipHE_LIKE][nelem][9][1].Emis->Aul ) /as[nelem] > 0.025 )
			{
				fprintf(ioQQQ,
	...
	if( !opac.lgCaseB )
	{
		...
		for( i = 0; i <=110; i++ )
		{
			...
			if( DrakeTotalAuls[i] > 0. && 
				i < iso.numLevels_max[ipHE_LIKE][ipHELIUM] - iso.nCollapsed_max[ipHE_LIKE][ipHELIUM])
			{
				if( fabs( DrakeTotalAuls[i] - (1./StatesElemNEW[ipHELIUM][ipHELIUM-ipHE_LIKE][i].lifetime) ) /DrakeTotalAuls[i] > 0.001 )
				{
					fprintf(ioQQQ,
						" SanityCheck found helium lifetime outside 0.1 pct of Drake values: index, expected, found: %li %.4e %.4e\n",
						i,
						DrakeTotalAuls[i], 
						(1./StatesElemNEW[ipHELIUM][ipHELIUM-ipHE_LIKE][i].lifetime) );
					lgOK = false;
				}
			}
		}
	...
	if( dense.lgElmtOn[ipHELIUM] )
	{
		...
		j = MIN2( NHE1CS+1 , iso.numLevels_max[ipHE_LIKE][ipHELIUM] -iso.nCollapsed_max[ipHE_LIKE][ipHELIUM] );
		for( n=1; n<j; ++n )
		{
			...
			if( fabs( he1cs[n-1] - opac.OpacStack[i - 1] ) /he1cs[n-1] > 0.15 )
			{
				...
				fprintf(ioQQQ,
					" n=%li, l=%li, s=%li\n",
					StatesElemNEW[ipHELIUM][ipHELIUM-ipHE_LIKE][n].n ,
					StatesElemNEW[ipHELIUM][ipHELIUM-ipHE_LIKE][n].l ,
					StatesElemNEW[ipHELIUM][ipHELIUM-ipHE_LIKE][n].S);
				lgOK = false;
			}
		}
	}
...
#	endif
	...
	for( nelem=0; nelem<2; ++nelem )
	{
		...
		{ 
			...
			Z4 *= Z4;
			/* H Lya */
			ans1 = Transitions[ipH_LIKE][nelem][ipH2p][ipH1s].Emis->Aul;
			ans2 = 6.265e8*Z4;
	...
	for( nelem=0; nelem<LIMELM; ++nelem )
	{
		...
		{
			int ipHi, ipLo;
			for( ipHi=4; ipHi< iso.numLevels_max[ipH_LIKE][nelem]-iso.nCollapsed_max[ipH_LIKE][nelem]; ++ipHi )
			{
				double sum = 0.;
				for( ipLo=0; ipLo<ipHi; ++ipLo )
				{
					sum += iso.BranchRatio[ipH_LIKE][nelem][ipHi][ipLo];
				}
				if( fabs(sum-1.)>0.01 ) 
				{
					fprintf(ioQQQ , 
	...
	for( nelem=0; nelem<LIMELM; ++nelem )
	{
		...
		{
			int ipHi, ipLo;
			for( ipHi=1; ipHi< iso.numLevels_max[ipH_LIKE][nelem]-iso.nCollapsed_max[ipH_LIKE][nelem]; ++ipHi )
			{
				...
				for( ipLo=0; ipLo<ipHi; ++ipLo )
				{
					sum += Transitions[ipH_LIKE][nelem][ipHi][ipLo].Emis->Aul;
				}
	...
	long ipISO = ipH_LIKE;
	nelem = 0;
	for( long n=1; n <= iso.n_HighestResolved_max[ipISO][nelem]; ++n )
	{
		double rel_photon_energy = 1. + FLT_EPSILON*2.;
		for( long l=0; l < n; l++ )
		{
			double cs;
			long index = iso.QuantumNumbers2Index[ipISO][nelem][n][l][2];

			cs = H_photo_cs( rel_photon_energy, n, l, nelem+1 );

			error = fabs(cs - opac.OpacStack[iso.ipOpac[ipISO][nelem][index]-1] )/
				( (cs + opac.OpacStack[iso.ipOpac[ipISO][nelem][index]-1] )/2.);

			if( error > 0.05 )
			{
				fprintf(ioQQQ , "SanityCheck finds insane H photo cs, n,l = %li, %li, expected, found = %e, %e, error = %e\n",
					n, l, opac.OpacStack[iso.ipOpac[ipISO][nelem][index]-1], cs, error );
				lgOK = false;
			}
		}
	}
	...
	for( nelem=0; nelem<LIMELM; ++nelem )
	{
		...
		if( dense.lgElmtOn[nelem] )
		{
			...
			hbn[ipH_LIKE][nelem] = iso.DepartCoef[ipH_LIKE][nelem][0];
			hn[ipH_LIKE][nelem] = StatesElemNEW[nelem][nelem-ipH_LIKE][0].Pop;
			xIonF[ipH_LIKE][nelem] = dense.xIonDense[nelem][nelem+1-ipH_LIKE];

			iso.DepartCoef[ipH_LIKE][nelem][0] = 0.;
			StatesElemNEW[nelem][nelem-ipH_LIKE][0].Pop = 1.;
			dense.xIonDense[nelem][nelem+1-ipH_LIKE] = 1.;

			if( nelem > ipHYDROGEN )
			{

				hbn[ipHE_LIKE][nelem] = iso.DepartCoef[ipHE_LIKE][nelem][0];
				hn[ipHE_LIKE][nelem] = StatesElemNEW[nelem][nelem-ipHE_LIKE][0].Pop;
				xIonF[ipHE_LIKE][nelem] = dense.xIonDense[nelem][nelem+1-ipHE_LIKE];

				/* this does not exist for hydrogen itself */
				iso.DepartCoef[ipHE_LIKE][nelem][0] = 0.;
				StatesElemNEW[nelem][nelem-ipHE_LIKE][0].Pop = 1.;
			...
			for( ion=0; ion<=nelem; ++ion )
			{
				...
				if( nelem > 1 )
				{
					realnum saveion[LIMELM+3];
					/* check that photoionization cross sections are ok */
					for( j=1; j <= (nelem + 2); j++ )
					{
						saveion[j] = dense.xIonDense[nelem][j-1];
						dense.xIonDense[nelem][j-1] = 0.;
					}
					...
					for( j=1; j <= (nelem + 2); j++ )
					{
						dense.xIonDense[nelem][j-1] = saveion[j];
					}

				}
			}
			iso.DepartCoef[ipH_LIKE][nelem][ipH1s] = hbn[ipH_LIKE][nelem];
			StatesElemNEW[nelem][nelem-ipH_LIKE][ipH1s].Pop = hn[ipH_LIKE][nelem];
			dense.xIonDense[nelem][nelem+1-ipH_LIKE] = xIonF[ipH_LIKE][nelem];

			if( nelem > ipHYDROGEN )
			{
				iso.DepartCoef[ipHE_LIKE][nelem][ipHe1s1S] = hbn[ipHE_LIKE][nelem];
				StatesElemNEW[nelem][nelem-ipHE_LIKE][ipHe1s1S].Pop = hn[ipHE_LIKE][nelem];
