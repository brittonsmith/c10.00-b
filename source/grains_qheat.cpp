...
#	endif
	...
	for( size_t nd=0; nd < gv.bin.size(); nd++ )
	{
		...
		long qnbin=-200;
		realnum threshold;
		vector<realnum> Invalid;
		vector<realnum>& grn = Invalid; // initialization is mandatory...
		double xx;

		/* save emission from this species */
		scase = gv.which_strg[gv.bin[nd]->matType];
		switch( scase )
		{
		case STRG_SIL:
			grn = gv.SilicateEmission;
			break;
		case STRG_CAR:
			grn = gv.GraphiteEmission;
			break;
		default:
			fprintf( ioQQQ, " GrainMakeDiffuse called with unknown storage class: %d\n", scase );
			cdEXIT(EXIT_FAILURE);
		}
