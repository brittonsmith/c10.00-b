...
void radius_increment(void)
{
	...
	for( long ipISO=ipH_LIKE; ipISO<NISO; ++ipISO )
	{
		for( nelem=ipISO; nelem<LIMELM; ++nelem)
		{
			if( dense.lgElmtOn[nelem] )
			{
				...
				{
					struc.StatesElemNEW[nelem][nelem-ipISO][level][nzone_minus_1] = (realnum)StatesElemNEW[nelem][nelem-ipISO][level].Pop;
				}
			}
		}
	}
	...
	colden.He123S += StatesElemNEW[ipHELIUM][ipHELIUM-ipHE_LIKE][ipHe2s3S].Pop*(realnum)radius.drad_x_fillfac;
