...
typedef enum {
	RFI_TABLE, OPC_TABLE, OPC_GREY, OPC_PAH1, OPC_PAH2N, OPC_PAH2C
} rfi_type;
...
STATIC void mie_cs(double,/*@in@*/const sd_data*,/*@in@*/const grain_data*,/*@out@*/double*,/*@out@*/double*,
		   /*@out@*/double*,/*@out@*/int*);
STATIC void car1_fun(double,/*@in@*/const sd_data*,/*@in@*/const grain_data[],/*@out@*/double*,/*@out@*/double*,
		     /*@out@*/double*,/*@out@*/int*);
STATIC void pah1_fun(double,/*@in@*/const sd_data*,/*@in@*/const grain_data*,/*@out@*/double*,/*@out@*/double*,
		     /*@out@*/double*,/*@out@*/int*);
STATIC void ld01_fun(double,/*@in@*/const sd_data*,/*@in@*/const grain_data[],/*@out@*/double*,/*@out@*/double*,
		     /*@out@*/double*,/*@out@*/int*);
...
STATIC void mie_repair(/*@in@*/const char*,long,int,int,/*@in@*/const realnum[],double[],/*@in@*/vector<int>&,
		       bool,/*@in@*/bool*);
STATIC double mie_find_slope(/*@in@*/const realnum[],/*@in@*/const double[],/*@in@*/const vector<int>&,
		   ...
		   long int nbin)
{
	...
	for( p=0; p < sd.nPart; p++ ) 
	{
		...
		for( i=0; i < rfield.nupper; i++ ) 
		{
			...
			case OPC_PAH2C:
				...
				for( gd2.cAxis=0; gd2.cAxis < gd2.nAxes; gd2.cAxis++ ) 
				{
					mie_cs_size_distr(wavlen,&sd,&gd,ld01_fun,&cs_abs,&cs_sct,&cosb,&Error);
					ErrorIndex[i] = max(ErrorIndex[i],Error);
					acs_abs[p][i] += cs_abs*gd2.wt[gd2.cAxis];
					acs_sct[p][i] += 0.1*cs_abs*gd2.wt[gd2.cAxis];
					a1g[p][i] += 0.1*cs_abs*1.*gd2.wt[gd2.cAxis];
	...
	for( i=0; i < rfield.nupper; i++ ) 
	{
		...
		for( p=0; p < sd.nPart; p++ ) 
		{
			lgErr = lgErr || ( fprintf(fdes,"%.6e ",a1g[p][i]) < 0 );
		}
		lgErr = lgErr || ( fprintf(fdes,"\n") < 0 );
	}
...
STATIC void car1_fun(double wavl,    /* in micron */
		     /*@in@*/ const sd_data *sd,
		     ...
		     /*@out@*/ int *error)
{
	DEBUG_ENTRY( "car1_fun()" );

	// this implements a variation of Eqs. 2 & 3 of LD01; it creates a gradual change from PAH-like behavior
	// at small radii (less than 50A) to graphite-like at large radii. The factor q_gra is set to zero

	const double q_gra = 0.00;
	const double a_xi = 50.e-4;
	pah1_fun(wavl,sd,&gdArr[0],cs_abs,cs_sct,cosb,error);
	if( sd->cSize > a_xi )
	{
		double xi_PAH = (1.-q_gra)*min(1.,pow3(a_xi/sd->cSize));
		double cs_abs2;
		// ignore cs_sct, cosb, and error from pah1_fun and return the graphite ones instead.
		// pah1_fun never returns errors and the other two values are ignored by the upstream code
		mie_cs(wavl,sd,&gdArr[1],&cs_abs2,cs_sct,cosb,error);
		*cs_abs = xi_PAH*(*cs_abs) + (1.-xi_PAH)*cs_abs2;
	}
	return;
}
...
STATIC void ld01_fun(double wavl,    /* in micron */
		     /*@in@*/ const sd_data *sd,
		     /*@in@*/ const grain_data gdArr[], /* gdArr[2] */
		     /*@out@*/ double *cs_abs,
		     /*@out@*/ double *cs_sct,
		     ...
		     /*@out@*/ int *error)
{
	...
	const double q_gra = 0.01;
	const double a_xi = 50.e-4;
	double xi_PAH, cs_abs1, cs_abs2;
	if( wavl >= 1./17.25 )
	{
		pah2_fun(wavl,sd,&gdArr[0],&cs_abs1,cs_sct,cosb,error);
		     ...
		     /*@out@*/ int *error)
{
	...
	else if( x < 17.26 )
	{
		...
		// graphite opacities should be used in that case; this is handled in mie_write_opc()
		TotalInsanity();
...
STATIC double mie_find_slope(/*@in@*/ const realnum anu[],
			 ...
			 /*@out@*/ grain_data *gd)
{
	...
	else
	{
		...
		fprintf( ioQQQ, " Allowed values are: RFI_TBL, OPC_TBL, GREY, PAH1, PH2N, PH2C\n");
		cdEXIT(EXIT_FAILURE);
	}
			  ...
			  /*@in@*/  long int *dl)
{
	...
	if( read_whole_line( chLine, FILENAME_PATH_LENGTH_2, io ) == NULL ) 
	{
		...
		fprintf( ioQQQ, " This grain opacity file does not have the expected format.\n");
		cdEXIT(EXIT_FAILURE);
	}
	(*dl)++;
	return;
