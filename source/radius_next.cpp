...
int radius_next(void)
{
	...
	if( nzone>1 )
	{
		 const double extin_mag_V_limit = 20.;
	...
	if( thermal.heating[0][22]/thermal.htot > 0.2 )
	{
		...
		if( Strong/thermal.htot > 0.1 )
		{
			...
			if( level == 1 )
				...
				/* a atom_level2 line was the heat source (bad case since not as well treated) */
				t = &TauLine2[ipStrong];
			else if( level == 3 )
				...
				/* 3rd party database transition */
				t = dBaseLines[ipStrong].tran;
			else
			{
	...
	if( nzone >= 4 )
	{
		...
		double dr_mole_abund = BigRadius;
		...
		{
			ostringstream oss;
			oss << "change in heavy elem mole abundance, d(mole)/elem=" << scientific << setprecision(2);
			oss << dCO_abund << " mole=" << mole_dr_change << "=" << COmole[mole_dr_change]->label;
			drChoice.insert( pair<const double,string>( dr_mole_abund, oss.str() ) );
		}
	}

	/* some consideration due to big H2 molecule */
	drChoice.insert( pair<const double,string>( H2_DR(), "change in big H2 Solomon rate line opt depth" ) );
