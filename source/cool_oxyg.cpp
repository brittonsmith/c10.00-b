...
void CoolOxyg(void)
{
	...
	***********************************************************************/
	...
	static vector< pair<transition*,double> > O4Pump;
	O4Pump.reserve(48);
	/* one time initialization if first call */
	if( O4Pump.empty() )
	{
		// set up level 1 pumping lines
		pair<transition*,double> pp( &TauLines[ipT789], 1./6. ); 
		O4Pump.push_back( pp );
...
#				endif
	...
	/* now sum pump rates */
	double pump_rate = 0.;
	vector< pair<transition*,double> >::const_iterator o4p;
	for( o4p=O4Pump.begin(); o4p != O4Pump.end(); ++o4p )
	{
		const transition* t = o4p->first;
