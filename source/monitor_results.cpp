	...
	FILE * ioMONITOR )
{
	...
	for(i=0; i<nAsserts; ++i )
	{
		...
		else if( strcmp( chAssertType[i] , "CB") == 0 )
		{
			...
			RelError[i] = 0.;
			long nHighestPrinted = iso.n_HighestResolved_max[nISOCaseB][nelemCaseB];
...
#				endif
		...
		else if( strcmp( chAssertType[i] , "D ") == 0 )
		{
			...
			else
			{
				RelError[i] = 0.;
				PredQuan[i] = 0.;
				long numPrintLevels = iso.numLevels_local[ipH_LIKE][ipZ] - (long)AssertQuantity2[i];
				for( n=(long)AssertQuantity2[i]; n<numPrintLevels+(long)AssertQuantity2[i]; ++n )
				{
		...
		else if( strcmp( chAssertType[i] , "De") == 0 )
		{
			...
			else
			{
				RelError[i] = 0.;
				PredQuan[i] = 0.;
				long numPrintLevels = iso.numLevels_local[ipHE_LIKE][ipZ] - (long)AssertQuantity2[i];
				for( n=(long)AssertQuantity2[i]; n<numPrintLevels+(long)AssertQuantity2[i]; ++n )
				{
					double relerror;
					relerror = 0.;
